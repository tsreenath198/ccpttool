...
import {
  ClientApplicationStatusModel,
  ActionsList
} from "./client-application-status.model";
...
export class ClientApplicationStatusComponent implements OnInit {
  ...
  public initialGetAll() {
    ...
    let temp = this.http.get(
      this.urlConstants.CASGetAll + pageNumber + "&pageSize=50&sortBy=id"
    );
    temp.subscribe(resp => {
      this.clientApplicationStatusList = resp as any;
  ...
  public trash(): void {
    ...
    const temp = this.http.delete(
      this.urlConstants.CASDelete + this.selectedRecrdToDel
    );
    temp.subscribe(
      resp => {
