...
export class MessageHistoryComponent implements OnInit {
  public model: MessageHistoryModel = <MessageHistoryModel>{};
  public ehList: any = [];
  ...
  private getById(id) {
    const temp = this.http.get(this.urlConstants.SMSHGetById + id);
    temp.subscribe(resp => {
  ...
  public update(searchBankForm: NgForm) {
    this.spinner(false);
    const temp = this.http.update(this.model, this.urlConstants.SMSHUpdate);
    temp.subscribe(
      resp => {
        this.formReset();
        this.toastr.success(
