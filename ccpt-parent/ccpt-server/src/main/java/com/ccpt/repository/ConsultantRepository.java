...
public interface ConsultantRepository extends BaseRepository<Consultant, Integer> {
	...
	@Query(value = "SELECT * FROM   consultant WHERE  (Upper(fullname) LIKE :searchKey OR Upper(email) LIKE :searchKey  OR phone LIKE :searchKey OR Upper(skills)  LIKE :searchKey ) AND active_flag = 1 ORDER  BY created_date DESC", nativeQuery = true)
	List<Consultant> search(@Param(value = "searchKey") String searchKey);

//	SELECT * FROM   consultant WHERE  (Upper(fullname) LIKE :searchKey OR Upper(email) LIKE :searchKey  OR phone LIKE :searchKey OR Upper(skills)  LIKE :searchKey ) AND active_flag = 1 ORDER  BY created_date DESC 
